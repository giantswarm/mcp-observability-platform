...
- All three MCP transports wired and selectable via `MCP_TRANSPORT`: `streamable-http` (default, OAuth-gated, remote), `sse` (OAuth-gated, remote; for SSE-only clients), `stdio` (local-dev / desktop-client integrations; no HTTP listener, no OAuth — the spawning process is fully trusted in the same model as kubectl). `cmd/serve.go` split into per-transport entrypoints so each transport's wiring stays readable.
