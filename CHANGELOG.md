...
- MCP `notifications/progress` for long-running tools: Prometheus/Loki/Tempo range queries and `get_panel_image` renders emit a `querying …` + `done` pair when the client attached a `progressToken`. Cancellation is handled by mcp-go natively via `ctx` cancellation through to Grafana HTTP calls — no code changes needed.
