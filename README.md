...
| `OAUTH_TRUSTED_ISSUERS`                     | no             | JSON array of external issuers (the muster broker) whose Bearer JWTs are accepted at `/mcp` alongside Dex, for agent OBO/M2M. Each entry: `issuer`, `jwksURL` (required), optional `allowedAudiences`, `allowedClaims`, `acceptedTypHeaders` (default `["at+jwt"]`; use `[""]` for K8s SA tokens), `allowPrivateIPJWKS`. Unlike trusted audiences these are full issuers verified via their own JWKS. |
